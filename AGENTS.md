...
- `app/lib/anthropic.ts`: Claude Sonnet 5 (adaptive thinking, `low` effort) via raw `fetch` (no SDK) — async generator yielding text with tool use loop (chat history, message search, WakaTime, Last.fm)
